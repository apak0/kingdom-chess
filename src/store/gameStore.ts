...
import { Piece, Position, Move, GameMode, PieceType } from '../types';
...
const convertBoardFromChess = (chess: Chess): (Piece | null)[][] => {
  ...
  for (let y = 0; y < 8; y++) {
    for (let x = 0; x < 8; x++) {
      const square = convertToChessNotation({ x, y });
      const piece = chess.get(square);
      board[y][x] = convertPieceFromChess(piece);
    }
  }
